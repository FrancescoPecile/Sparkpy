...
spark = SparkSession.builder.appName("myApp").master("local[*]") \
    .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/Spark.wall_impressions30") \
    .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/Spark.wall_impressions30") \
...
df7 = df6.filter('EVENT_TYPE=="wall-impression"').withColumn('DATE', col('ISOTIMESTAMP').cast('date')) \
    ...
    .count().withColumnRenamed("count", "IMPRESSIONS30").cache()

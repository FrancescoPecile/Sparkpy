...
spark = SparkSession.builder.appName("myApp").master("local[*]") \
    .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/Spark.wall_impressions29") \
    .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/Spark.wall_impressions29") \
...
spark.sparkContext.setJobGroup("id", "s3")

day_added = datetime.timedelta(days=1)

today_datetime = datetime.datetime(2021, 6, 29)
...
df7 = df6.filter('EVENT_TYPE=="wall-impression"').withColumn('DATE', col('ISOTIMESTAMP').cast('date')) \
    ...
    .withColumnRenamed("BRAND", "BRAND29")\
    .withColumnRenamed("WALL_ID", "WALL_ID29")\
    .withColumnRenamed("CAMPAIGN_ID", "CAMPAIGN_ID29")\
    .withColumnRenamed("EVENT_TYPE", "EVENT_TYPE29") \
    .withColumnRenamed("WALLGROUP_ID", "WALLGROUP_ID29") \
    .cache()

# df7.write.format("mongo").mode("append").save()
